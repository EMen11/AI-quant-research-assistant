"""Legacy live Streamlit application preserved during the Block 1 migration."""
...
st.caption(
    "This tool is for research and educational purposes only and does not constitute "
    "financial advice."
)
...
query = st.text_area(
    ...
    placeholder=(
        "e.g. Should I increase my exposure to Swiss pharmaceutical stocks given recent "
        "FDA approvals?"
    ),
    height=100,
)
        ...
        try:
            ...
            with tab4:
                ...
                for ticker, weight in weights.items():
                    st.progress(
                        float(weight),
                        text=f"{ticker}: {round(float(weight) * 100, 1)}%",
                    )
